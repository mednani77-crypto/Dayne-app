...
object PdfStatementService {
    ...
    fun generateStatementPdf(
        ...
        fun drawHeader(c: Canvas) {
            ...
            c.drawText(strings.appName + " – " + (if (businessName.isNotBlank()) businessName else strings.appTagline), titleX, margin + 28f, paint)

            paint.textSize = 12f
            paint.typeface = regularTypeface
            c.drawText(strings.statementTitle + " (" + statementData.currencyCode + ")", titleX, margin + 50f, paint)
        ...
        fun drawPartyInfoBox(c: Canvas, startY: Float): Float {
            ...
            // Financial Summary on Right Side (or Left if RTL)
            paint.textAlign = if (isRtl) Paint.Align.LEFT else Paint.Align.RIGHT
            val rightColX = if (isRtl) margin + 12f else pageWidth - margin - 12f

            val openBalFormatted = AmountFormatter.formatAmount(statementData.openingBalance, statementData.decimalPlaces, statementData.currencyCode)
            c.drawText("${strings.statementInitialBalance} $openBalFormatted", rightColX, currY + 22f, paint)

            val debtsFormatted = AmountFormatter.formatAmount(statementData.periodTotalDebts, statementData.decimalPlaces, statementData.currencyCode)
            c.drawText("${strings.statementTotalDebts} $debtsFormatted", rightColX, currY + 40f, paint)

            val payFormatted = AmountFormatter.formatAmount(statementData.periodTotalPayments, statementData.decimalPlaces, statementData.currencyCode)
            c.drawText("${strings.statementTotalPayments} $payFormatted", rightColX, currY + 58f, paint)

            paint.typeface = boldTypeface
            paint.textSize = 11f
            val closeBalFormatted = AmountFormatter.formatAmount(statementData.closingBalance, statementData.decimalPlaces, statementData.currencyCode)
            paint.color = if (statementData.closingBalance >= 0) Color.parseColor("#00796B") else Color.parseColor("#D32F2F")
            c.drawText("${strings.statementClosingBalance} $closeBalFormatted", rightColX, currY + 80f, paint)
        ...
        fun drawTableHeader(c: Canvas, startY: Float): Float {
            ...
            c.drawRect(margin, hY, pageWidth - margin, hY + 24f, paint)
            ...
            if (isRtl) {
                ...
                c.drawText(strings.dateLabel, pageWidth - margin - 8f, hY + 16f, paint)
                c.drawText(strings.statementDescription, pageWidth - margin - 80f, hY + 16f, paint)
                paint.textAlign = Paint.Align.LEFT
                c.drawText(strings.statementDebtCol, margin + 190f, hY + 16f, paint)
                c.drawText(strings.statementPaymentCol, margin + 105f, hY + 16f, paint)
                c.drawText(strings.statementRunningBalanceCol, margin + 8f, hY + 16f, paint)
            } else {
                paint.textAlign = Paint.Align.LEFT
                c.drawText(strings.dateLabel, margin + 8f, hY + 16f, paint)
                c.drawText(strings.statementDescription, margin + 80f, hY + 16f, paint)
                paint.textAlign = Paint.Align.RIGHT
                c.drawText(strings.statementDebtCol, pageWidth - margin - 190f, hY + 16f, paint)
                c.drawText(strings.statementPaymentCol, pageWidth - margin - 105f, hY + 16f, paint)
                c.drawText(strings.statementRunningBalanceCol, pageWidth - margin - 8f, hY + 16f, paint)
            }
            return hY + 28f
        }
        ...
        fun drawFooter(c: Canvas, pageNum: Int) {
            ...
            c.drawText("${strings.statementFooterText} • Page $pageNum", pageWidth / 2f, pageHeight - margin + 15f, paint)
        }
        ...
        // Save PDF to cache dir with safe filename
        val safeName = statementData.party.name.replace(Regex("[^a-zA-Z0-9_\\-\\u0600-\\u06FF]"), "_")
        val fileName = "DeynBook_${safeName}_${statementData.currencyCode}_${DateFormatter.formatForFileName(System.currentTimeMillis())}.pdf"

    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = {
                    Column {
                        Text(strings.collectionCenter, fontWeight = FontWeight.Bold)
                        Text(strings.collectionSubtitle, style = MaterialTheme.typography.bodySmall)
                    }
                },
                navigationIcon = {
        ...
        LazyColumn(
            ...
            item {
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    FilterChip(selected = filter == CollectionFilter.ALL, onClick = { filter = CollectionFilter.ALL }, label = { Text(strings.collectionCenter) })
                    FilterChip(selected = filter == CollectionFilter.OVERDUE, onClick = { filter = CollectionFilter.OVERDUE }, label = { Text(strings.overdue) })
                    FilterChip(selected = filter == CollectionFilter.TODAY, onClick = { filter = CollectionFilter.TODAY }, label = { Text(strings.dueToday) })
                    FilterChip(selected = filter == CollectionFilter.SOON, onClick = { filter = CollectionFilter.SOON }, label = { Text(strings.dueSoon) })
                }
            }
            ...
            if (filtered.isEmpty()) {
                ...
                items(filtered, key = { it.debtTransactionId }) { item ->
                    CollectionCard(
                        item = item,
                        onOpen = { onOpenParty(item) },
                        onWhatsApp = { onWhatsAppReminder(item) },
        ...
        Column(modifier = Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
            Row(
                ...
                Column(modifier = Modifier.weight(1f)) {
                    Text(item.party.name, style = MaterialTheme.typography.titleMedium, fontWeight = FontWeight.Bold)
                    item.party.phone?.let { Text(it, style = MaterialTheme.typography.bodySmall) }
                }
                Text(
                    AmountFormatter.formatAmount(item.outstandingAmountMinor, item.decimalPlaces, item.currencyCode),
            ...
            Row(verticalAlignment = Alignment.CenterVertically) {
                ...
                Text(
                    "${strings.dueDate}: ${DateFormatter.formatDate(item.dueAt)} • $statusText",
            ...
            Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                ...
                Button(onClick = onPayment, modifier = Modifier.weight(1f)) {
                    Icon(Icons.Default.AccountBalanceWallet, contentDescription = null)
                    Text(strings.recordPayment, modifier = Modifier.padding(start = 5.dp))
                }
            }
        }
    }
